 ...
 */
template <template <class K, class V> class Dict>
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test) // 15min
{
    /**
     * @todo Implement this function! You should use the provided
     * templated hashtable class Dict.
     */
     Dict<char, int> wordHash(17);
     Dict<char, int> testHash(17);
     bool ret=true;
    ...
    for(size_t i=0;i<word.length();i++)
    {
        if(wordHash[word[i]]!=testHash[word[i]])
            return false;
    }
    for(size_t i=0;i<test.length();i++)
    {
        if(wordHash[test[i]]!=testHash[test[i]])
            return false;
    }
