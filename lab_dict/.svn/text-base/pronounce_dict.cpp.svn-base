...
{
    string up_word1 = word1;
    string up_word2 = word2;
    std::transform(up_word1.begin(), up_word1.end(), up_word1.begin(), ::toupper);
    std::transform(up_word2.begin(), up_word2.end(), up_word2.begin(), ::toupper);
    auto look1 = dict.find(up_word1);
    auto look2 = dict.find(up_word2);
    if(look1 != dict.end() && look2 != dict.end())
    { 
        if(look1->second == look2->second)
        return true;
        else return false;
    }
    else return false;
